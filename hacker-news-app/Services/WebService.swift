...
class WebService {
    ...
    func getStoriesById(storyId: Int) -> AnyPublisher<Story, Error> {
    ...
    private func mergeStories(ids storyIds: [Int]) -> AnyPublisher<Story, Error> {
        ...
        let initialPublisher = getStoriesById(storyId: storyIds[0])
        let remainder = Array(storyIds.dropFirst())
        
        return remainder.reduce(initialPublisher) { combined, id in
            return combined.merge(with: getStoriesById(storyId: id))
                .eraseToAnyPublisher()
        }
    }
